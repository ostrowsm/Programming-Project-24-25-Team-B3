...
class DailyDelaysGraph {
  ...
  void drawGraph() {
    ...
    float hitRadius = 10;
    background(255); // clear the background to redraw
    // Draw x and y axes 
    stroke(0); // Set the line color to black
    line(margin, height - margin, margin, margin); // Y-axis, line(startX, startY, endX, endY)
    ...
    for (int day = 1; day < numberOfDays; day++) {
      ...
      float y = map(delayCounts.get(day - 1), 0, maxDelayCount, height - margin, margin);

      // check if the mouse is close to this point
      if (dist(mouseX, mouseY, x, y) < hitRadius) {
        fill(0); // used for setting color for shapes, in this case, the color is black
      ...
      fill(0);
      textAlign(CENTER);
      // + 20: Adds 20 pixels to the height - margin value, effectively moving the text 20 pixels down from the x-axis line.
      text(String.valueOf(day), x, height - margin + 20);
      text("Day of Month", width/2, height-margin/3);
      ...
      if (day > 1) {
        ...
        */
        float prevY = map(delayCounts.get(day - 2), 0, maxDelayCount, height - margin, margin);
    ...
    textAlign(RIGHT);
    text("0", margin - 10, height - margin);
    text(String.valueOf(maxDelayCount), margin - 10, margin);
  }
