...
void draw() {
  ...
  String[] airCode = {"JAX", "TVC", "RAP", "CDC", "STC", "KTN", "BRW", "HNL", "STL", "XNA", "MHK", "STT", "STS", "DFW", "PNS", 
  ...
  int barWidth = 1100 / airCode.length;

  int[] intervalCounts = new int[airCode.length]; // array to store counts for each interval
  ...
  for (int i = 0; i < intervalCounts.length; i++) {
    ...
    fill(0, 0, 255);
    textAlign(CENTER);
    text(intervalCounts[i], x + barWidth / 2, 770);
  }
